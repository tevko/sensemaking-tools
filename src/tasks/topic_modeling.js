...
exports.LEARN_TOPICS_AND_SUBTOPICS_PROMPT = exports.LEARN_TOPICS_PROMPT = void 0;
exports.learnSubtopicsPrompt = learnSubtopicsPrompt;
...
`;
exports.LEARN_TOPICS_AND_SUBTOPICS_PROMPT = `
...
`;
function learnSubtopicsPrompt(parentTopics) {
    const parentTopicNames = parentTopics.map((topic) => topic.name).join(", ");
    return `
 ...
 */
function generateTopicModelingPrompt(includeSubtopics, parentTopics) {
    if (!includeSubtopics) {
        return exports.LEARN_TOPICS_PROMPT;
    }
    else if (parentTopics === null || parentTopics === void 0 ? void 0 : parentTopics.length) {
        return learnSubtopicsPrompt(parentTopics);
    }
    else {
        return exports.LEARN_TOPICS_AND_SUBTOPICS_PROMPT;
    }
}
/**
 ...
 */
function learnedTopicsValid(response, parentTopics) {
    ...
    if (parentTopics) {
        const allowedTopicNames = parentTopics.map((topic) => topic.name).concat("Other");
